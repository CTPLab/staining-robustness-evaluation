...
def Wagner2023(
    pretrained_path: str = "../models/WAGNER2023/MSI_high_CRC_model.pth",
) -> nn.Module:
    transformer = Transformer(num_classes=1)
    ckpt = torch.load(pretrained_path, map_location="cpu")
    e = transformer.load_state_dict(
        {k.replace("model.", ""): v for k, v in ckpt.items() if "criterion.pos_weight" not in k}
    )
    print(e)
    return transformer
